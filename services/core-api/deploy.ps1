...
if ($apiId -eq "None") {
  $apiId = aws apigatewayv2 create-api --region $AwsRegion --name $apiName --protocol-type HTTP --cors-configuration "AllowOrigins=['*'],AllowMethods=['GET','POST','PUT','DELETE','OPTIONS'],AllowHeaders=['authorization','content-type','x-workspace-id','x-user-id','x-sub-account-id']" --tags Project=calbook,App=calbook --query ApiId --output text
  ...
  aws apigatewayv2 update-api --region $AwsRegion --api-id $apiId --cors-configuration "AllowOrigins=['*'],AllowMethods=['GET','POST','PUT','DELETE','OPTIONS'],AllowHeaders=['authorization','content-type','x-workspace-id','x-user-id','x-sub-account-id']" | Out-Null
}
...
Push-Location "$root\apps\admin-web"
$env:VITE_API_BASE_URL = $apiUrl
npm.cmd run build
Pop-Location
aws s3 sync "$root\apps\admin-web\dist" "s3://$bucketName" --delete
